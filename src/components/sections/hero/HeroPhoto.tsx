...
function HeroPhoto() {
  const { multipleSplats } = useSimulation();
  const heroCurveRef = useRef<SVGSVGElement>(null);
  const heroDrawRef = useRef<HTMLDivElement>(null);
  const heroImageRef = useRef<HTMLImageElement>(null);

  function heroCurveDone() {
    if (!heroCurveRef.current || !heroDrawRef.current || !heroImageRef.current)
      return;

    heroDrawRef.current.classList.add('hero-opacity');
    heroImageRef.current.classList.add('hero-opacity');
    heroCurveRef.current.classList.add('hero-gradient');

    heroDrawRef.current.addEventListener(
      'transitionend',
      () => {
        if (!heroCurveRef.current || !heroDrawRef.current) return;
        heroDrawRef.current.classList.remove('hero-opacity');
        heroCurveRef.current.classList.remove('hero-gradient');
        heroCurveRef.current.classList.remove('hero-opacity');
        multipleSplats(7);
      },
      { once: true },
    );
