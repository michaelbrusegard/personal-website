...
const ServiceCard = ({ index, title, icons }: { index: number; title: string; icons: any }) => (
  <Tilt className='pointer-events-auto w-full xs:w-[250px]' scale={1.1} transitionSpeed={450} tiltMaxAngleX={15} tiltMaxAngleY={15}>
    ...
    >
      <h3 className='text-center font-mono text-[20px] font-bold'>{title}</h3>
      <div className='flex min-h-[280px] flex-col justify-evenly rounded-[20px] bg-secondary px-8 py-5'>
        {icons.map((icon: any) => (
          ...
          >
            ...
            <span className='ml-2 py-1 font-mono text-sm font-medium'>{icon.name}</span>
          </div>
        ))}
      </div>
    </motion.div>
...
const About = () => {
  return (
    <>
      ...
      <motion.p variants={fadeIn('', '', 0.1, 1) as Variants} className='section-text pointer-events-auto mt-4 max-w-3xl text-[17px] leading-[30px]'>
        As a full-stack developer and a computer science student, I bring a strong foundation in TypeScript, JavaScript, Python, and Java. Throughout my
        journey, I&apos;ve worked with various frameworks like Django, Next.js, Spring Boot and Express.js. I&apos;m a quick learner, I thrive on collaboration
